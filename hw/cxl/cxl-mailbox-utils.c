                                          ...
                                          CXLCCI *cci)
{
    ...
    for (i = 0; i < in->num_entries_updated; i++) {
        ...
        QTAILQ_FOREACH(ent, &ct3d->dc.extents_pending_to_add, node) {
            /* If the host does not accept any extents, do we need to remove
             * the extents in the pending extent list?
             * How do we know the response is for a specific adding request?
             * */
            if (ent->start_dpa <= dpa &&
                dpa + len <= ent->start_dpa + ent->len) {
                break;
